...
const Notifications = () => {
    ...
    const unreadNotification = inboxNotifications.filter((notification) => !notification.readAt)
            ...
            <PopoverContent align={"end"} className={"shad-popover"}>
                <LiveblocksUIConfig overrides={{
                    ...
                    <InboxNotificationList>
                        ...
                        {unreadNotification.length > 0 && unreadNotification.map((notification) => (
                            <InboxNotification key={notification.id} inboxNotification={notification} className={"bg-dark-200 text-white"} href={`/documents/${notification.roomId}`} showActions={false} kinds={{thread: (props) => (<InboxNotification.Thread {...props} showActions={false} showRoomName={false}/>), textMention:(props) => (<InboxNotification.TextMention {...props}  showRoomName={false}/>), $documentAccess: (props) => (
